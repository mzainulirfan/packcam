...
import { getSessionTtlHours } from '../http'
...
const SESSION_TTL_MS = Math.max(1, getSessionTtlHours()) * 60 * 60 * 1000
...
export function createSession(operatorName: string, operatorCode: string, role: OperatorRole, taskType: WorkTask, persistent = false) {
  const sessionId = makeId('session')
  const timestamp = nowIso()
  db().prepare(
    `INSERT INTO operator_sessions (session_id, operator_name, operator_code, role, task_type, persistent, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(sessionId, operatorName, operatorCode, role, taskType, persistent ? 1 : 0, timestamp, timestamp)
...
export function getSessionById(sessionId: string) {
  const row = db()
    .prepare(
      `SELECT session_id, operator_name, operator_code, role, task_type, persistent, created_at, updated_at
...
export function authenticateOperator(input: {
  ...
  const session = createSession(profile.operatorName, profile.operatorCode, profile.role, profile.taskType, input.persistent === true)
  return {
    session,
    profile,
  }
