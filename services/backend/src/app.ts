...
app.post('/api/packing-sessions/merge', requireSession, (req, res) => {
  try {
    const ids = Array.isArray(req.body?.sessionIds) ? (req.body.sessionIds as unknown[]) : []
    const merged = mergePackingSessions(ids.map((id) => String(id)))
    return sendOk(res, merged)
  } catch (error) {
    return sendError(res, 400, error instanceof Error ? error.message : 'Gagal menggabungkan sesi.')
  }
})
...
app.get('/api/dashboard/summary', requireSession, (req, res) => {
  const query = req.query as Record<string, string | string[] | undefined>
  return sendOk(res, getDashboardSummary(readQueryString(query.date)))
})
...
app.delete('/api/orders/by-order/:orderNumber', requireSession, (req, res) => {
  try {
    const params = req.params as Record<string, string | undefined>
    return sendOk(res, deleteShopeeOrderByOrderNumber(params.orderNumber ?? ''))
  } catch (error) {
    return sendError(res, 400, error instanceof Error ? error.message : 'Gagal menghapus order Shopee.')
  }
})
...
app.patch('/api/recordings/:id/packing-pay-rule', requireAdmin, (req, res) => {
  try {
    const params = req.params as Record<string, string | undefined>
    const updated = updatePackingRecordingPayRule(params.id ?? '', readStringField(req.body?.ruleId, 'ruleId'))
    return sendOk(res, updated)
  } catch (error) {
    return sendError(res, 400, error instanceof Error ? error.message : 'Gagal mengubah pay rule recording.')
  }
})
