        ...
        <div className="grid gap-4 p-4 sm:p-5">
          ...
          <div className="overflow-x-auto scrollbar-thin rounded-xl border border-[#dddddd]">
            <table className="w-full min-w-[860px] border-collapse">
              ...
              <tbody className="divide-y divide-[#e6e6e6]">
                {payRules.map((r) => (
                  <tr key={r.id} className="bg-white hover:bg-[#fbfaf9]"><Td className="font-medium text-[#000000]">{r.name}</Td><Td>{r.matchType}{r.matchValue ? `:${r.matchValue}` : ''}</Td><Td>{r.payType}</Td><Td><InlineNumber defaultValue={r.amount} className="w-[90px]" onCommit={(v) => { if (v !== r.amount && Number.isFinite(v) && v > 0) void handleInlineUpdateRule(r, { amount: v }) }} /></Td><Td><InlineNumber defaultValue={r.priority} className="w-[70px]" onCommit={(v) => { if (v !== r.priority && Number.isFinite(v)) void handleInlineUpdateRule(r, { priority: v }) }} /></Td><Td><Button type="button" variant={r.active ? 'default' : 'outline'} size="sm" className={`h-8 rounded-full px-3 text-[12px] font-medium ${r.active ? 'bg-[#0075de] text-white hover:text-white hover:bg-[#005bab]' : 'border-[#dddddd] bg-white text-[#615d59]'}`} onClick={() => void handleToggleRuleActive(r)}>{r.active ? 'Aktif' : 'Nonaktif'}</Button></Td><Td className="text-right"><SmallAction onClick={() => void handleDeletePayRule(r.id)}><HugeiconsIcon icon={Delete02Icon} size={14} strokeWidth={1.9} /></SmallAction></Td></tr>
                ))}
