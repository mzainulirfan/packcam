...
export function useWatermarkedStream({
  ...
  useEffect(() => {
    ...
    function drawFrame() {
      ...
      if (video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA && video.videoWidth > 0 && video.videoHeight > 0) {
        ...
        drawingContext.fillText(
          `${(watermarkTaskRef.current ?? 'qc').toUpperCase()} | ${watermarkOperatorRef.current ?? '-'}`,
          watermarkX + 16,
          watermarkY + 38,
          watermarkWidth - 32,
        )
  ...
  const commitWatermarkFrame = (nextWatermarkResi?: string | null) => {
    if (typeof nextWatermarkResi !== 'undefined') {
      watermarkResiRef.current = nextWatermarkResi
    }

    drawFrameRef.current?.()
  }

  return { stream: watermarkedStream, commitWatermarkFrame }
}
